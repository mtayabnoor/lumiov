  ...
  if (!socket) {
    ...
    socket.on('disconnect', (reason) => {
      ...
      if (reason === 'io server disconnect' || reason === 'transport close') {
        window.dispatchEvent(new CustomEvent('global-error', { detail: `Backend disconnected: ${reason}` }));
      }
    });
    ...
    socket.on('connect_error', (err) => {
      ...
      window.dispatchEvent(new CustomEvent('global-error', { detail: `Connection error: ${err.message}` }));
    });
  }
  return socket;
};
