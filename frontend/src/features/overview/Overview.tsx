...
function Overview() {
  ...
  const healthyDeployments = deployments.filter((d) => d.status?.readyReplicas === d.spec?.replicas).length;
      ...
      >
        <Grid container spacing={3} sx={{ mb: 4 }}>
          <Grid size={{ xs: 6, sm: 4, md: 2 }}>
            <SummaryCard title="Total Pods" value={pods.length} loading={podsLoading} color="#3b82f6" />
          </Grid>
          <Grid size={{ xs: 6, sm: 4, md: 2 }}>
            <SummaryCard title="Running" value={runningPods} loading={podsLoading} color="#22c55e" />
          </Grid>
          <Grid size={{ xs: 6, sm: 4, md: 2 }}>
            <SummaryCard title="Pending" value={pendingPods} loading={podsLoading} color="#f59e0b" />
          </Grid>
          <Grid size={{ xs: 6, sm: 4, md: 2 }}>
            <SummaryCard title="Failed" value={failedPods} loading={podsLoading} color="#ef4444" />
          </Grid>
          <Grid size={{ xs: 6, sm: 4, md: 2 }}>
            <SummaryCard title="Deployments" value={deployments.length} loading={depsLoading} color="#8b5cf6" subtitle={`${healthyDeployments} healthy`} />
          </Grid>
          <Grid size={{ xs: 6, sm: 4, md: 2 }}>
            <SummaryCard title="Containers" value={totalContainers} loading={podsLoading} color="#06b6d4" />
          </Grid>
        </Grid>
